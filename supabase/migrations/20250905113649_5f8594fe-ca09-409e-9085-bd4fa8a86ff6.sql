-- 1) Update booking_status enum to include all required states
alter type booking_status add value if not exists 'bestaetigt';
alter type booking_status add value if not exists 'abgelehnt';
alter type booking_status add value if not exists 'abgesagt';
