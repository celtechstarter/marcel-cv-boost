...
export const RelatedPages = ({ currentPage }: RelatedPageProps) => {
  const { t } = useI18n();
  ...
  const relatedPages = allPages.filter(page => 
    page.relevantFor.includes(currentPage) && page.id !== currentPage
  ).slice(0, 3);

  if (relatedPages.length === 0) return null;

  return (
    <section className="bg-gradient-to-r from-secondary/20 to-accent/20 py-16 border-t border-border/50">
      <div className="container mx-auto max-w-6xl px-4">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {relatedPages.map((page) => (
            <Card key={page.id} className="card-soft hover:shadow-lg transition-all duration-300 group">
