...
const Navigation = () => {
  ...
  const navItems = [{
    ...
    href: "/#services",
    label: t('navigation.items.services'),
    title: t('navigation.titles.services')
  }, {
    href: "/#ablauf",
    label: t('navigation.items.process'),
    title: t('navigation.titles.process')
  }, {
    ...
    href: "/blog",
    label: t('navigation.items.blog'),
    title: t('navigation.titles.blog')
  }, {
    href: "/tools",
    label: t('navigation.items.tools'),
    title: t('navigation.titles.tools')
  }];
  ...
  return <>
      {/* Skip Link */}
      <a href="#main" className="sr-only focus:not-sr-only focus:absolute focus:top-2 focus:left-2 focus:z-[60] focus:px-4 focus:py-2 focus:bg-primary focus:text-primary-foreground focus:rounded-md focus:font-medium">
        {t('navigation.skipLink')}
      </a>

      <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-background/80 backdrop-blur-md border-b border-border/50 shadow-md' : 'bg-background/80 backdrop-blur-md border-b border-border/50'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {isOpen && <div id="mobile-menu" className="lg:hidden animate-fade-in motion-reduce:animate-none" role="dialog" aria-modal="true" aria-label="Hauptmenü">
              <div className="px-2 pt-2 pb-3 space-y-1 bg-background/95 backdrop-blur-md rounded-lg mt-2 shadow-medium border border-border/50">
                {navItems.map(item => <a key={item.href} href={item.href} title={item.title} className={`block px-3 py-2 text-foreground/80 hover:text-primary hover:bg-muted/50 transition-colors duration-300 font-medium rounded-md focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2 ${isActiveRoute(item.href) ? 'text-primary bg-muted border-l-4 border-primary' : ''}`} onClick={handleMenuItemClick} aria-current={isActiveRoute(item.href) ? "page" : undefined}>
                    {item.label}
                  </a>)}
                <div className="pt-2">
                  <Button variant="default" className="btn-accent w-full" asChild>
                    <a href="/bewerbungshilfe#anfrage" onClick={handleMenuItemClick} aria-label={t('navigation.titles.help')}>
                      {t('navigation.requestButton')}
                    </a>
                  </Button>
                </div>
